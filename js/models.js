...
class Story {
  ...
  getHostName() {
    return this.url.split("/").filter(str => str.includes('.'))[0];
  }
}
...
class User {
  ...
  async toggleFavoriteStory(story,method){
    console.log(method)
    await axios({
    ...
    method === "POST" ? this.favorites.push(story) : this.favorites = this.favorites.filter(s => s.storyId !== story.storyId);
  }

  starFav() {
    this.favorites.forEach(story => {
      const $trgt = $(`#${story.storyId}`).children('i')
      starToggle($trgt)
    })
  }
}